...
use frame_support::{decl_module, decl_storage, decl_event, decl_error, ensure, StorageValue, StorageMap, dispatch, traits::Get, traits::Randomness};
...
decl_storage! {
	trait Store for Module<T: Trait> as KittiesModule {
		...
		pub KittiesOwners get(fn kitties_owners): map hasher(blake2_128_concat) KittyIndex => Option<T::AccountId>;
	}
}
